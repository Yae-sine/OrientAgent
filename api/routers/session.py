...
@router.post("/{session_id}/select-filiere")
async def select_filiere(session_id: str, selection: FiliereSelection):
    """
    Select a filière from the top 3 for interview simulation.
    """
    if session_id not in _sessions:
        raise HTTPException(status_code=404, detail="Session not found")
    
    session = _sessions[session_id]
    top_3 = session.get("state", {}).get("top_3", [])
    
    # Find the selected filière
    selected = None
    for f in top_3:
        if f.get("filiere_id") == selection.filiere_id:
            selected = f
            break
    
    if not selected:
        raise HTTPException(status_code=400, detail="Filière not in top 3 recommendations")
    
    # Update session state
    session["state"]["filiere_choisie"] = selected.get("filiere_nom", "")
    
    # Generate interview questions
    from agents.coach import CoachEntretienAgent
    from graph.nodes import _get_llm
    
    coach = CoachEntretienAgent(llm=_get_llm())
    questions = await coach.generate_questions(
        filiere_nom=selected.get("filiere_nom", ""),
        filiere_context=selected.get("justification", ""),
        langue=session["state"].get("langue", "fr")
    )
    
    session["state"]["interview_questions"] = questions
    session["state"]["current_step"] = "interview_active"
    
    return {
        "filiere_selected": selected.get("filiere_nom"),
        "questions": questions,
        "total_questions": len(questions),
    }


@router.post("/{session_id}/interview/answer", response_model=InterviewEvaluation)
async def submit_interview_answer(session_id: str, request: InterviewAnswerRequest):
    """
    ...
    evaluation = await coach.evaluate_answer(
        ...
        filiere_nom=state.get("filiere_choisie", ""),
        serie_bac=state.get("serie_bac", "Sciences"),
        interets=state.get("interets", [])
    )
    
    # Store answer and evaluation
    if "interview_answers" not in state:
        state["interview_answers"] = []
    state["interview_answers"].append(request.answer)
    
    if "interview_evaluations" not in state:
        state["interview_evaluations"] = []
    state["interview_evaluations"].append(evaluation)
    
    # Check if interview is complete
    is_complete = len(state["interview_answers"]) >= len(questions)
    
    if is_complete:
        # Compute final score
        final_feedback = coach.compute_final_score(state["interview_evaluations"])
        state["interview_score"] = final_feedback["score"]
        state["interview_feedback"] = final_feedback
        state["current_step"] = "complete"
        
        # Regenerate PDF with interview results
        from pdf.generator import generate_report
        pdf_path = generate_report(state)
        state["pdf_path"] = pdf_path
        
        _save_session(session_id, state, "complete")
    
    return InterviewEvaluation(
        clarte=evaluation.get("clarte", 5),
        motivation=evaluation.get("motivation", 5),
        connaissance=evaluation.get("connaissance", 5),
...
@router.get("/{session_id}/interview/questions")
async def get_interview_questions(session_id: str):
    """
    Get the interview questions for the current session.
    """
    if session_id not in _sessions:
        raise HTTPException(status_code=404, detail="Session not found")
    ...
    return {
        "filiere": state.get("filiere_choisie", ""),
        "questions": questions,
